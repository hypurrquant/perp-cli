...
QA cycle `qa/2026-05-16-numeric-audit-test-followup` — 39 commits. The first numeric-validation block closes test-coverage gaps from the v0.13.0 / numeric-validation-audit cycles, adds a shared NaN/empty-string guard helper across all 4 adapters, and surfaces the missing `liquidationPrice` in `perp portfolio`; **verified end-to-end against mainnet** (60 read-only CLI commands inside `perp-qa` Docker container, 4 DEX, zero false positives). Later commits add sizing / utils / Lighter coverage, fix two latent arb-sizing / `symbolMatch` defects surfaced while adding those guards, and stop an abort-listener leak in the polling loops. A final block closes a risk-control gap surfaced by a code audit: **manual orders now enforce the local risk limits** (`trade market/buy/sell/limit`), with a `--force` bypass. (All work after the numeric block is unit + `tsc` verified, not part of the 60-command live run.) Full QA report: `docs/qa-reports/2026-05-16-numeric-audit-test-followup.md`.
...
- **Manual-order risk gate** — `enforceOrderRisk()` in `src/trade-validator.ts`, invoked by `trade market/buy/sell/limit` immediately after the dry-run guard. Resolves notional (limit price, or markPrice fetched via `getMarkets`), runs `assessRisk` + `preTradeCheck`, and throws `RISK_VIOLATION` (with a remediation hint) when the order would breach `~/.perp/risk.json` limits. `reduce-only` orders and the new `--force` flag short-circuit before any fetch. Covered by `src/__tests__/enforce-order-risk.test.ts` (9 cases).
- **`--force` flag** on `trade market/buy/sell/limit` — bypasses the local risk gate for an intentional override.
...
- **Manual entry orders now enforce local risk limits** (previously advisory-only). `trade market/buy/sell/limit` called the adapter directly, so `maxPositionUsd` / `maxTotalExposureUsd` / `maxPositions` in `~/.perp/risk.json` were ignored for hand-placed orders — only auto-trading (`cross-chain-margin`) and the `trade check` command honored them. They are now enforced on every manual entry order, in **both TTY and JSON modes**, unless `--force` is passed or the order is `reduce-only`. Defaults remain loose (`maxPositionUsd` 100000) so most orders are unaffected until tightened via `perp risk set`. **Automated callers that place large orders via `perp --json trade ...` may now receive `RISK_VIOLATION`** — pass `--force` or raise the limit. Not yet applied to `twap/scale-in/multi/split/stop/tpsl` (follow-up).
...
### Test
- **Unit suite**: 1400 → **1535 passed** (+135 across 82 files).
